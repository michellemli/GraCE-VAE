...
def laplacian_smoothness(W, edge_index, edge_weight=None, normalize=True):
    """
    W: [p, h] decoder final layer weight (p genes)
    edge_index: [2, E] with nodes in [0, p-1]
    edge_weight: [E] optional
    """
    src, dst = edge_index[0], edge_index[1]  # [E], [E]
    diff = W[src] - W[dst]                   # [E, h]
    sq = (diff * diff).sum(dim=1)            # [E]
    if edge_weight is not None:
        sq = sq * edge_weight
    loss = sq.sum()
    if normalize:
        denom = (edge_weight.sum() if edge_weight is not None else sq.numel())
        loss = loss / (denom + 1e-12)
    return loss
...
def get_data(
    ...
    if mode == 'train':
        ...
        train_idx, val_idx,test_idx = split_scdata(
            dataset, 
    ...
    edim = getattr(dataset, 'embedding_dim', 0)
    ...
    if mode == 'train':
        ...
        dataset3 = Subset(dataset, val_idx)
        ptb_name = dataset.ptb_names[val_idx]
        dataloader1 = DataLoader(
            dataset3,
            batch_sampler=SCDATA_sampler(dataset3, batch_size, ptb_name),
            num_workers=0
        )
        return dataloader, dataloader1, dataloader2, dim, cdim, ptb_genes, edim
    else:	
        dataloader = DataLoader(
            dataset,
